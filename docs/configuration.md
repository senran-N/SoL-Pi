...
- `observationPack`: registers `obs_recall`, archives large text-only tool results under the session-derived runtime root, and projects a bounded placeholder after the first two provider requests. Once a placeholder has been projected, the next completed turn appends Pi 0.87's branch-local `context_edit` for that tool-result entry. The raw session message remains unchanged, the edit is replayable on resume and branch-local, and a failed archive or target lookup fails open to the original result.
