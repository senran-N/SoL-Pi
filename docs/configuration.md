...
- `observationPack`: registers `obs_recall` and `obs_search`, archives large text-only tool results under the session-derived runtime root, and projects a bounded placeholder when explicit tool intent, a stable projection, or window pressure makes packing worthwhile. Without a reliable horizon it keeps the original result visible. Once a placeholder has been projected, the next completed turn appends Pi 0.87's branch-local `context_edit` for that tool-result entry. The raw session message remains unchanged, the edit is replayable on resume and branch-local, and a failed archive or target lookup fails open to the original result.
...
The assistant can also ask for a window directly with `new_context` and check the budget with `get_context_remaining`. The budget report uses the same conservative token estimate as the compaction gate, derives its percentage from that count, and falls back to the active model's window when Pi has no valid window report. Its `token_source` identifies whether the selected estimate came from Pi or the local projection; neither is an exact tokenizer count. Corrections and steering clear stale task forecasts without erasing unpaid cache-rebuild debt. A requested reset is honored even when the economic gate would decline, but it is applied at the next idle settlement so the tool call never aborts the turn that made it, and it is declined outright when the session has nothing Pi could archive. Each compaction appends one audit line to `<runtimeRoot>/online-context-compact/windows.jsonl`. Windows are numbered by recorded compaction, not by context reset, so a correction that rebuilds the plan without producing a checkpoint never leaves a gap in the ids or makes a fragment point at a predecessor that does not exist. The fragment carries only a bounded index of durable notes; immutable note bodies are content-addressed under `<runtimeRoot>/online-context-compact/notes/objects/` and the current branch's note versions are recorded in session entries, then read back on demand with `note_read`. `history_search`/`history_read` make the same guarantee for the session log itself: they read only the entries Pi already recorded on the current branch, including work a compaction removed from the window but never a branch that was forked or rewound away from, and they bound both a search answer (4 KB) and a single read (24 KB) so recalling the past cannot refill the window. Observation Pack uses the same Pi context-edit boundary when a large result is packed: the stored tool result remains available to `obs_recall` and `obs_search`, while the active branch records only the replacement content, so native compaction and resume do not restore the large payload merely because the request-count ledger was not replayed.

## Images and model input limits

Image handling remains Pi's responsibility. Pi 0.87 normalizes prompt and tool-result images before they enter the session/provider path, applies the active model's `inputLimits.images.resize` profile when configured, and honors Pi's `images.blockImages` setting. SoL-Pi does not resize, base64-reencode, omit, or archive image content in Observation Pack; mixed or image-only tool results therefore stay unchanged by that mechanism. Configure image limits with Pi's model/provider settings, not `sol-pi.json`. The active model's `inputLimits.maxRequestBytes`, `inputLimits.images.maxPerMessage`, and `inputLimits.images.maxPerRequest` are model metadata available for provider/runtime enforcement; SoL-Pi only uses its own text-result placeholder policy and conservative context estimates.
...
What comes back is checked. Every citation names a path, a line, and a quote, and it survives only when that quote is found at that line at delivery time. A rejected citation is reported as rejected; an answer that claims a finding and has no surviving citation is refused outright rather than handed to the agent as a plausible summary. An exploration that runs out of steps, times out, or cannot resolve its model fails the tool call, which leaves the agent to do the search itself. The step-by-step transcript and content-addressed observations are written to `<runtimeRoot>/scoped-exploration/`; sensitive observations are deliberately omitted and the result reports an incomplete audit when that happens.
...
An explicit `timeout` in the shell tool call keeps Pi's destructive meaning and reaches Pi's backend unchanged; the yield deadline is separate and never terminates anything. Before a yield, interrupting the turn kills the process tree through Pi's own teardown. After a yield the command belongs to its handle, so a later turn's interrupt does not reach it; `exec_kill` and session shutdown do. The session-derived `command-yield/<handle>.log` spool stores output only with restrictive permissions, and a bounded read cache keeps memory use predictable; command lines are never written because a command line can carry a credential.

## Pi integration

SoL-Pi reads no dedicated environment variables. Evidence-Preserving Reducer resolves its configured reducer provider/model through `ExtensionContext.modelRegistry` and uses Pi-managed authentication. Identical diagnostic results can reuse a bounded, session-local verified receipt without a new model call; see [compatibility](compatibility.md#evidence-preserving-reducer) for cache lifetime and accounting. On a cache miss, if the configured reducer model is unavailable or the nested model call fails, the original tool result continues unchanged.
