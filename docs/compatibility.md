...
SoL-Pi is developed and tested against `@earendil-works/pi-coding-agent` 0.87.0. On both releases the full test suite, type checking, package inspection, and the public API check pass, and nothing in the suite is skipped: a guard that would otherwise need a privileged filesystem operation uses an equivalent one the platform does allow, so it is exercised rather than stepped over. The suite is 32 files and 288 tests at the time of writing.
...
Pi 0.87.0's `ExtensionContext.compact()` aborts the active agent before it summarizes, and `agent_settled` fires only once a whole run has drained every turn, retry, auto-compaction, and queued continuation. A plan boundary that selects compaction therefore saves its plan and progress state, calls `ExtensionContext.abort()` to stop the run, and runs compaction from the `agent_settled` that stop produces. The handler awaits the compaction's own `onComplete`/`onError` callbacks. On success, the extension sends a hidden reminder through public `ExtensionAPI.sendMessage()` with `triggerTurn: true`, so Pi starts a new turn against the compacted context and rebuilds the plan even when the native summary omits that instruction.

Pi 0.87.0 defers runs requested from `agent_settled` handlers until all settled handlers finish, then awaits each deferred action inside `_emitAgentSettled` before resolving the outer prompt. This means `sendMessage({triggerTurn:true})` queues a continuation that Pi manages internally — the handler returns immediately after sending, and the deferred run starts after the emit completes. No explicit settlement barrier is needed because Pi's deferred-action loop keeps the process alive until the continuation finishes.

The synchronous turn-start guard that verified continuation startup by checking `isIdle()` after `sendMessage()` has been removed because `isIdle()` is always `true` during the handler in 0.87.0. The `isIdle()` guard at the `agent_settled` entry — which skips compaction when a run is still active — was also removed because `agent_settled` only fires when the session is idle.

This continuation is armed only by a successful boundary compaction. Cancelling or exiting does not schedule one. A Pi build that never emits `agent_settled` starts no boundary compaction.
