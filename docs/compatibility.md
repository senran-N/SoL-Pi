...
SoL-Pi is developed and tested against `@earendil-works/pi-coding-agent` 0.87.0. The full test suite, type checking, package inspection, and public API check pass, and nothing in the suite is skipped: a guard that would otherwise need a privileged filesystem operation uses an equivalent one the platform does allow, so it is exercised rather than stepped over. The current suite is 33 files and 292 tests.
...
Pi 0.87's `turn_end` event accepts append-only boundary drafts and commits them before the next provider request. Online Context Compact uses that path for structured window resets: it appends a retain-none compaction entry (`firstKeptEntryId: null`), a hidden continuation, and the updated state in one boundary, so the same `AgentSession` continues without aborting the run or requesting a summary from a model. Explicit `new_context` uses the same path after confirming Pi has history it can archive.
